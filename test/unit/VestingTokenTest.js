...
describe("VestingToken", function () {
    ...
    it ('test prevent user from releasing if vesting are revoked', async function() {
        ...
        await vestingToken.revoke(investor.address)

        await expect(vestingToken.connect(investor).release()).to.be.revertedWith("Vesting revoked");
    })
    ...
    it('test prevent user from transferring locked tokens', async function() {
        ...
        await expect(vestingToken.connect(investor).transfer(publicSale.address, totalSupply*40n /100n)).to.be.revertedWith('Cannot transfer locked tokens')
    })
